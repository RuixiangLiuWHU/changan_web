...
export default {
  ...
  methods: {
    slope() {
      ...
      let url2 = '/geocube8085/oge-dag/hillShade?level=' + this.level + '&spatialRange=' + spatialRange
    ...
    luojia() {
      ...
      var str = '{"0": {"functionInvocationValue": {"functionName": "CoverageCollection.addStyles", "arguments": {"input": {"functionInvocationValue": {"functionName": "CoverageCollection.binarization", "arguments": {"coverageCollection": {"functionInvocationValue": {"functionName": "CoverageCollection.subCollection", "arguments": {"filter": {"functionInvocationValue": {"functionName": "Filter.equals", "arguments": {"leftField": {"constantValue": "crs"}, "rightValue": {"constantValue": "EPSG:4326"}}}}, "input": {"functionInvocationValue": {"functionName": "Service.getCoverageCollection", "arguments": {"baseUrl": {"constantValue": "http://localhost"}, "bbox": {"constantValue": [73.62, 18.19, 134.7601467382, 53.54]}, "datetime": {"constantValue": ["2018-09-15 14:39:50", "2018-09-15 14:39:55"]}, "productID": {"constantValue": "LJ01_L2"}}}}}}}, "threshold": {"constantValue": 1}}}}, "max": {"constantValue": 255}, "min": {"constantValue": 0}}}}}'
      let url1 = '/geocube8085/oge-dag/saveDagJson'
      //form data传参
      if (this.dagbool) {
        this.level = getZoomLevel(this.viewer)
        this.windowRange = getCesiumViewerExtend(this.viewer)
        ...
        console.log(this.windowRange)
        var spatialRange = this.windowRange.minx + ',' + this.windowRange.miny + ',' + this.windowRange.maxx + ',' + this.windowRange.maxy
        let url2 = '/geocube8085/oge-dag/runDagJson?level=' + this.level + '&spatialRange=' + spatialRange
        axios.post(url2).then(
          (response) => {
            console.log('提交请求成功！')
            for (let obj of this.layers) {
